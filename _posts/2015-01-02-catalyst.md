...
严格来说，这个部分和Catalyst没有直接关系。因为Spark和Catalyst都是由Scala实现的，我们很自然的选择Scala内建的Parser Combinators。
...
以上都是我在看Catalyst的时候关注的点，因为我觉得实现一个DSL，我需要知道这些东西。而其它的细节，基本都可以在实现的过程中慢慢再看。其实，我实现写不下去了:)（笑）
